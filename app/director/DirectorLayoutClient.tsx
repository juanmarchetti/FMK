  ...
  return (
    <aside className="hidden lg:flex flex-col w-[272px] min-h-screen border-r border-[#54585B]/20 bg-[#F3F4F5] p-5 shrink-0">
      <div className="mb-8 pb-5 border-b border-[#54585B]/15">
        ...
        <p className="mt-1.5 text-xs text-[#54585B]">Panel de Director</p>
      </div>
          ...
          return (
            <Link key={item.href} href={item.href}
              className={`flex h-11 items-center gap-3 rounded px-3 text-sm font-semibold transition ${
                active ? "bg-[#7A1F2A] text-white" : "text-[#54585B] hover:bg-white hover:text-[#191C1D]"
              }`}>
      ...
      <div className="mt-6 rounded-lg border border-[#7A1F2A]/25 bg-white p-4">
        <p className="text-xs font-bold uppercase tracking-wide text-[#7A1F2A]">Normativa activa</p>
        <p className="mt-1.5 text-sm font-semibold text-[#191C1D]">Grados FMK 2017 v1.1</p>
...
function MobileHeader({ title, onMenu }: { title: string; onMenu: () => void }) {
  return (
